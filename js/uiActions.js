...
function refreshDefaultVariantDropdown(){
 const dd=document.getElementById('defaultVariant');
 if(!dd) return;
 dd.innerHTML='';
 document.querySelectorAll('.variant-row').forEach((row,i)=>{
   const name=row.querySelector('.variant-name')?.value||`Variant ${i+1}`;
...
window.refreshDefaultVariantDropdown=refreshDefaultVariantDropdown;
