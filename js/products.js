...
function renderProductsTable() {
  ...
  products.forEach(product => {
    const lowStock = Number(product.stock || 0) <= Number(product.reorderLevel || 0);
...
function renderPOSProducts() {
  ...
  products.forEach(product => {
    ...
    card.className = `pos-product-card ${lowStock ? 'low-stock' : ''} ${outOfStock ? 'out-of-stock' : ''}`;

    card.innerHTML = `
      <div class="pos-card-top">
        <div class="pos-category-badge">${escapeHtml(product.category)}</div>
        ${lowStock ? `<div class="pos-low-stock-pill">Low</div>` : ''}
      </div>
      ...
      <div class="pos-product-footer">
        ...
        <div class="pos-product-stock">${product.stock} left</div>
      </div>
    `;
