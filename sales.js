...
function openHeldOrdersModal(){
 ...
 list.innerHTML=held.length?held.map((o,i)=>`<div><button data-held-index="${i}" class="resume-held-btn">Resume</button></div>`).join(''):'No held orders';
 modal.style.display='block';
}
function closeHeldOrdersModal(){const m=document.getElementById('heldOrdersModal'); if(m)m.style.display='none';}
function resumeHeldOrder(index){
 const held=Array.isArray(APP_STATE.heldOrders)?APP_STATE.heldOrders:[];
 const order=held[index];
 if(!order)return;
 updateState('cart',()=>order.items||[]);
 held.splice(index,1);
 updateState('heldOrders',()=>held);
 renderCart(); updateCartSummary(); renderHeldOrdersBadge(); closeHeldOrdersModal();
 showNotification('Held order resumed','success');
}
